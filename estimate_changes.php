<?php
    ...
    if(isset($_REQUEST['delete_estimate_id'])) {
        ...
        if(!empty($delete_estimate_id)) {	
            ...
            $order_form_unique_id = "";
            $order_form_unique_id = $obj->getTableColumnValue($GLOBALS['order_form_table'], 'conversion_id', $delete_estimate_id, 'id');
            if(preg_match("/^\d+$/", $estimate_unique_id)) {
                ...
                if(empty($delete_access_error)) {
                    $stock_delete = '';
                    $stock_delete = $obj->DeletePrevList($delete_estimate_id, '');
