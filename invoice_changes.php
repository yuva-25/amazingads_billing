<?php
                            ...
                            <select class="select2 select2-danger"  name="bank_id" data-dropdown-css-class="select2-danger" style="width: 100%;">
                                ...
                                <?php if(!empty($bank_list)) {
                                    foreach($bank_list as $data) { ?>
                                        <option value="<?php if(!empty($data['bank_id'])) { echo $data['bank_id']; } ?>" <?php if(!empty($data['bank_id']) && ($data['bank_id'] == $bank_id))  { ?>selected<?php } ?>><?php if(!empty($data['bank_name'])) { echo $data['bank_name']; } ?></option>
                                <?php }
                                } ?>
                            </select>
                    ...
                    if(!empty($product_ids) && empty($invoice_error) && empty($valid_invoice)) {
                        for($j=0; $j < count($product_ids); $j++) {
                            ...
                            if(!empty($product_amount[$j])) {
                                ...
                                $tax_value = ($discounted_product_amount * $tax) / 100;
                                $individual_array[] = array('tax'=>$tax, 'quantity'=>$product_quantity[$j], 'taxable_value'=>$discounted_product_amount, 'tax_amount'=>$tax_value,'hsn_code'=>$hsn_code[$j]);
                            }
                        }
                        if(!empty($extra_charges_value)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                else
                {
                    ...
                    if(preg_match("/^\d+$/", $invoice_update_id)) {
                        $invoice_id = $edit_id; $update_stock = 1;
                        $invoice_number = $obj->getTableColumnValue($GLOBALS['invoice_table'], 'invoice_id', $invoice_id, 'invoice_number');
    ...
    if(isset($_REQUEST['delete_invoice_id'])) {
        ...
        if(!empty($delete_invoice_id)) {	
            ...
            $estimate_unique_id = "";
            $estimate_unique_id = $obj->getTableColumnValue($GLOBALS['estimate_table'], 'conversion_id', $delete_invoice_id, 'id');
            if(preg_match("/^\d+$/", $invoice_unique_id)) {
                ...
                if(empty($delete_access_error)) {
                    $stock_delete = '';
                    $stock_delete = $obj->DeletePrevList($delete_invoice_id, '');
                    if(empty($stock_delete == '1')) {
                            
                            if(!empty($estimate_unique_id)){
                                $columns = array(); $values = array();
                            ...
                            $estimate_id = "";
                            $estimate_id = $obj->getTableColumnValue($GLOBALS['invoice_table'], 'invoice_id', $delete_invoice_id, 'estimate_id');

                           if(!empty($estimate_id)){
                                $stock_unique_ids= array();
                                $stock_unique_ids = $obj->getAllRecords($GLOBALS['stock_table'], 'bill_unique_id', $estimate_id,'');
                                if(!empty($stock_unique_ids)) {
                                    foreach($stock_unique_ids as $val) {
                                        $stock_unique_id = "";
                                        if(!empty($val['id']) && $val['id'] != $GLOBALS['null_value']) {
